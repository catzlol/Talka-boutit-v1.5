...
$threadID = $_GET['id']; // Fetch thread ID from query parameters
...
if ($findThread->num_rows > 0) {
    $threadRow = (object) $findThread->fetch_assoc();
    $points = $threadRow->points; // Get the points for the thread

    $userID = $_SESSION['user_id']; // Assumes user ID is stored in session

    $voteQuery = "SELECT * FROM `thread_votes` WHERE `thread_id` = '$threadIDSafe' AND `user_id` = '$userID'";
    $voteResult = $conn->query($voteQuery);
    $userVote = $voteResult->num_rows > 0 ? (object) $voteResult->fetch_assoc() : null;

    $hasVoted = $userVote !== null;
    $currentVote = $hasVoted ? $userVote->vote_type : null;

    ?>
    <p>Points: <?php echo htmlspecialchars($points); ?></p> <!-- Display points -->
    <div>
        <?php echo bbcode_to_html($threadRow->content); ?>
    </div>

    <!-- Voting options -->
    <div>
        <?php if (!$hasVoted): ?>
            <a href="/forum/vote?action=upvote&thread_id=<?php echo $threadIDSafe; ?>" class="vote-button">Upvote</a>
            <a href="/forum/vote?action=downvote&thread_id=<?php echo $threadIDSafe; ?>" class="vote-button">Downvote</a>
        <?php else: ?>
            <a href="/forum/vote?action=remove_vote&thread_id=<?php echo $threadIDSafe; ?>" class="vote-button">Remove Vote</a>
        <?php endif; ?>
    </div>
